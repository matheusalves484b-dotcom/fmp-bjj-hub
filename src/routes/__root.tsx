...
function ErrorComponent({ error, reset }: { error: Error; reset: () => void }) {
  ...
  return <div className="flex min-h-screen items-center justify-center bg-background px-4"><div className="max-w-md text-center"><h1 className="text-xl font-semibold tracking-tight text-foreground">Não foi possível carregar</h1><p className="mt-2 text-sm text-muted-foreground">Algo deu errado. Tente atualizar a página.</p><div className="mt-6 flex flex-wrap justify-center gap-2"><button onClick={() => { router.invalidate(); reset(); }} className="inline-flex items-center justify-center rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground">Tentar novamente</button><a href="/" className="inline-flex items-center justify-center rounded-md border border-input bg-background px-4 py-2 text-sm font-medium text-foreground">Início</a></div></div></div>;
}
...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { title: "Gestão FMP BJJ" },
      { name: "description", content: "Gestão inteligente para academias de artes marciais." },
      { name: "author", content: "Gestão FMP BJJ" },
      { property: "og:title", content: "Gestão FMP BJJ" },
      { property: "og:description", content: "Gestão inteligente para academias de artes marciais." },
...
function RootComponent() {
  ...
  const hasGlobalSidebar = !isPublicRoute && location.pathname !== "/" && location.pathname !== "/tv";

  return <QueryClientProvider client={queryClient}>
    {isPublicRoute ? <Outlet /> : <AuthGuard>
      {hasGlobalSidebar ? <div className="min-h-screen md:pl-[232px]"><AppSidebar /><Outlet /></div> : <Outlet />}
    </AuthGuard>}
  </QueryClientProvider>;
}
